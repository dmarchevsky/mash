...
9. For each regression test listed in the feature file, design a test that verifies the existing behavior is preserved.
10. Follow the project's test naming conventions from architecture.md. Mirror the source structure (e.g., `src/foo.py` → `tests/test_foo.py`).

### Phase 3 — Test Execution

11. Write and run tests one criterion at a time:
...
12. After all acceptance tests, write and run regression tests.
13. Run the full test suite once at the end to confirm everything together:
...
14. Append a `## QA outcome` section to the feature file with:
...
15. Update the feature file status:
