...
- **Always use AskUserQuestion.** When you need user input — choices, confirmations, or clarifications — use the AskUserQuestion tool. Never just print a question as text. Required fields for each question: `question` (the text), `header` (short label, max 12 chars), `options` (2-4 choices), `multiSelect` (true/false). Required fields for each option: `label` (concise choice text) and `description` (what it means or what happens if chosen). **Question type matters:** for choice questions (pick a strategy, confirm an action), options should be the actual choices. For open-ended questions (describe a problem, explain goals, provide details), options should be example suggestions — the user can always select "Other" to type a free-text answer. Never reduce an open-ended question to Yes/No.
